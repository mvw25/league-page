<script>
    ...
    export let keeperRow, draftType, row, reversalRound, previous=false, players, year, leagueTeamManagers;
</script>
...
<Row>
    {#each keeperRow as draftCol, col}
            ...
            <Cell class="draftCell{draftCol ? ' changedHands' : ''}{previous ? ` prev${players[draftCol.player].pos}` : ''}">
                <span class="draftPos{previous ? "Prev" : ""}">
                    ...
                    {:else if draftType == "snake" && !reversalRound}
                        {row}.{row % 2 == 0 ? keeperRow.length - col : col + 1}{draftCol?.newOwner ? ` ${getTeamNameFromTeamManagers(leagueTeamManagers, draftCol.newOwner, year)}` : ''}
                    {:else if draftType == "snake" && reversalRound}
                    ...
                    {:else}
                        ...
                        {:else}
                            {row}.{keeperRow.length - col}{draftCol?.newOwner ? ` ${getTeamNameFromTeamManagers(leagueTeamManagers, draftCol.newOwner, year)}` : ''}
                        {/if}
                    {/if}
                </span>
